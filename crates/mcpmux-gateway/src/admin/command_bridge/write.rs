...
pub async fn update_workspace_binding(
    ...
    let icon = normalize_icon(&body.icon);
    ...
    if existing.icon != updated.icon {
        maybe_remove_orphaned_icon(ctx, existing.icon.as_deref()).await?;
    }
...
pub async fn delete_workspace_binding(ctx: &AdminBridgeCtx, id: String) -> Result<Value> {
    let id_uuid = Uuid::parse_str(&id)?;
    let existing = ctx.workspace_binding_repository.get(&id_uuid).await?;
    ctx.workspace_binding_repository.delete(&id_uuid).await?;
    if let Some(binding) = existing {
        maybe_remove_orphaned_icon(ctx, binding.icon.as_deref()).await?;
    }
    Ok(json!({ "ok": true }))
}
...
pub async fn upload_workspace_icon(ctx: &AdminBridgeCtx, body: UploadIconBody) -> Result<Value> {
    let source = PathBuf::from(&body.source_path);
    let metadata = tokio::fs::metadata(&source)
        .await
        .with_context(|| format!("failed to inspect source file: {}", source.display()))?;
    if metadata.len() > MAX_UPLOAD_BYTES {
        return Err(anyhow!("icon file must be 2MB or smaller"));
    }

    let bytes = tokio::fs::read(&source).await?;
    let image = image::load_from_memory(&bytes).context("failed to decode image file")?;
    ...
    let icon_dir = ctx.data_dir.join(WORKSPACE_ICON_DIR);
    ...
    normalized
        ...
        .context("failed to store icon file")?;

    as_json(format!("{LOCAL_ICON_PREFIX}{file_name}"))
}
