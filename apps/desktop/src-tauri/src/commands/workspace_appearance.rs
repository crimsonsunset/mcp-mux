...
pub(crate) async fn maybe_remove_orphaned_icon_file(
    ...
    let appearance_still_referenced = appearances.iter().any(|a| a.icon == icon_ref_owned);
    ...
    let bindings = state
        ...
        .await
        .map_err(|e| e.to_string())?;
    let binding_still_referenced = bindings
        .iter()
        .any(|b| b.icon.as_deref() == Some(icon_ref_owned.as_str()));

    if appearance_still_referenced || binding_still_referenced {
        return Ok(());
    }
