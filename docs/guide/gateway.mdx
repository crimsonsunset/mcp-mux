...
For homelab operators who need the full McpMux UI from another device (phone, laptop) without screen sharing, enable **Web admin mode** in **Settings → Gateway**:
...
- **Cloudflare Access** (recommended): expose `mux.joe-hassio.com` via `cloudflared` to `localhost:45819` and enable **Trust Cloudflare Access JWT** so only authenticated tunnel traffic reaches the app
...
The Tauri app resolves `apps/desktop/dist` when the admin server starts. MCP clients should continue using `http://localhost:45818/mcp` (or `mcp.joe-hassio.com` through a separate tunnel hostname) — do not mix admin UI traffic onto the MCP gateway port.

See [Web Admin Mode planning](/docs/planning/web-admin-remote-access) for architecture and security notes.
