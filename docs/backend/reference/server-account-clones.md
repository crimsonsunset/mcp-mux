...
McpMux installs servers once per `(space_id, server_id)` — enforced by `UNIQUE(space_id, server_id)` on `installed_servers`, `credentials`, and `outbound_oauth_clients`. That model works when one Space maps to one account (Personal vs Work vs GAIT), but breaks down when a user needs **two accounts for the same MCP in the same Space**:
