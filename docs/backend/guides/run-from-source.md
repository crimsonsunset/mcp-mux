...
**Frontend boundary:** Browser code imports `@/lib/backend` (data commands via `apiCall`, live updates via `backend/events`, OS integrations via `backend/shell`). Do not import `@tauri-apps/*` outside `lib/backend/**` — ESLint enforces this. Deprecated `@/lib/api/*` shims still work during transition. See [`unified-backend-facade.md`](../../frontend/reference/unified-backend-facade.md).
...
- [`docs/../../frontend/reference/unified-backend-facade.md`](../../frontend/reference/unified-backend-facade.md) — three-channel frontend boundary (data / events / shell)
