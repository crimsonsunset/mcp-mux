...
| 8 | Reachable ≠ loaded | **Binding never auto-surfaces.** A bound bundle's tools become *invokable* + *discoverable* (via `search_tools`), not promoted into `tools/list`. Surfacing stays an explicit, small, human-curated choice (`surfaced: true` on FS members). | The whole `search → schema → invoke` design exists to keep startup context tiny. Persistent binding must not reintroduce the all-defs-in-context problem it was built to solve. A workspace can bind a huge bundle and startup context stays ~15 meta tools + a curated few surfaced. |
| 9 | No catalog firehose | **Remove `mcpmux_list_all_tools` from the agent surface.** Superseded by `search_tools` (scoped/paginated) + `list_servers` (roster) + `list_feature_sets` (bundles). | Its schema is tiny at startup, but a single *call* dumps the entire catalog (855KB-scale) into context as a tool result — and the name invites agents to call it. The three discovery tools cover every job it did, more cheaply. Full dumps remain a desktop/admin diagnostic, not an agent tool. |
...
- `mcpmux_search_tools` **defaults to active/bound scope** (lean). It *can* match **inactive** servers/feature sets, but only via an explicit `include_inactive: true` — inactive matches annotated `{ status: "inactive", bindable_feature_set_id }`.
