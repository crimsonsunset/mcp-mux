...
**Status:** Implemented on `root-resolution` — Phases 1–4. Inbound 404 body unchanged (mcp-remote / TS SDK do not re-init).
...
Traced live (code + the Case B run, not guessed):

- [`ServerConfigUpdatedHandler`](../../crates/mcpmux-gateway/src/consumers/server_config_handler.rs) already exists and **evicts only**. `handle_config_updated` calls `pool_service.remove_instance()` for enabled servers and returns. `RoutingService::call_tool` then fails with `format_server_bound_offline_error` (`routing.rs` — "bound but not connected") because a missing instance is not a reconnect trigger. [`clone-auth-header-config-editing.md`](./clone-auth-header-config-editing.md) Decision 4 asked for evict **+ reconnect**; the July 23 write-up is stale on "no subscriber" and on `retry_connection` being stubbed (`LiveGatewayWriteRuntime::retry_connection` already uses `reconnect_fresh`). After a Configure/definition/`UserSpaceSync` save, the next invoke still dies until something else reconnects.
- `PoolService::connect_server` still returns early when `is_healthy()` is true. Evict-only is why a save can even be seen; without a follow-up `reconnect_fresh`, the healthy-reuse lie just becomes a "not connected" lie.
- `reconnect_instance` → `reconnect_after_oauth` still warns `"Unexpected STDIO transport for OAuth reconnection, defaulting to HTTP"` and builds an HTTP transport (`connection.rs`). `call_tool` no longer uses that path for transport-closed errors, but `read_resource` / `get_prompt` still do on auth errors, and a future caller can still trip it.
...
- After that reload, a multi-folder Cursor window reports ~6 `roots/list` entries with no pin → resolver `PendingRoots` → every server `bindable`. `X-Mcpmux-Workspace` already shadows roots **once pinned**, but pin is skipped when the header arrives without `mcp-session-id` (initialize) or when the header is empty. `mcpmux_set_workspace_root` is a session disambiguation tool, not a reconnect tool.
...
initialize POST
  ...
  mcp-session-id: absent     ← today: pin skipped
        │
        ▼
...
| [`docs/planning/clone-auth-header-config-editing.md`](./clone-auth-header-config-editing.md) | One-line status: Decision 4 evict shipped; reconnect lives here |

---

## Phases

### Phase 1 — Config save reconnects (~2–3 hours)
...
### Phase 2 — Stdio cannot go through OAuth reconnect (~1–2 hours)
...
### Phase 3 — Inbound 404 after process death (~2 hours)
...
### Phase 4 — Trust a non-empty workspace header (~half day)
...
| File | Notes |
| ---- | ----- |
| [`crates/mcpmux-gateway/src/consumers/server_config_handler.rs`](../../crates/mcpmux-gateway/src/consumers/server_config_handler.rs) | Evict-only today (`remove_instance` L93). Phase 1 target |
| [`crates/mcpmux-gateway/src/pool/service.rs`](../../crates/mcpmux-gateway/src/pool/service.rs) | `reconnect_fresh` L470; `reconnect_instance` still OAuth-only L435; `read_resource`/`get_prompt` still call `reconnect_instance` L140/L214 |
| [`crates/mcpmux-gateway/src/pool/connection.rs`](../../crates/mcpmux-gateway/src/pool/connection.rs) | Stdio→HTTP fallback L506–512 — Phase 2 delete |
| [`crates/mcpmux-gateway/src/pool/transport/resolution.rs`](../../crates/mcpmux-gateway/src/pool/transport/resolution.rs) | `resolve_auto_connection_context` — already shared by routing + admin `retry_connection` |
| [`crates/mcpmux-gateway/src/admin/write_runtime.rs`](../../crates/mcpmux-gateway/src/admin/write_runtime.rs) | `retry_connection` already implemented (clone-auth doc is stale) |
| [`crates/mcpmux-gateway/src/mcp/oauth_middleware.rs`](../../crates/mcpmux-gateway/src/mcp/oauth_middleware.rs) | Pin skip when session id missing L273; 404 treated as expected noise L337–339 |
| [`crates/mcpmux-gateway/src/services/session_roots.rs`](../../crates/mcpmux-gateway/src/services/session_roots.rs) | `set_pinned` shadows `roots/list` |
...
- [`docs/planning/clone-auth-header-config-editing.md`](./clone-auth-header-config-editing.md) — Decision 4 (pool invalidation). Evict half shipped; reconnect is Phase 1 here. Clone UI/header work stays there
