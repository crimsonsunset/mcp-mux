...
### 4. Web admin clone parity (active — Phase 5 in `clone-auth-header-config-editing.md`)

Clone auth is meant to work identically from the web admin UI, not just desktop — this is **not** parked. `clone_server`, `is_clone_id_available`, `suggest_clone_suffix`, `list_clone_dependents`, and `set_server_display_name` all return `"...not yet available"` in [`command_bridge/write.rs`](../../crates/mcpmux-gateway/src/admin/command_bridge/write.rs) / [`command_bridge/read.rs`](../../crates/mcpmux-gateway/src/admin/command_bridge/read.rs), even though every `ServerAppService` method they need already exists and is already called by desktop Tauri's [`server_clone.rs`](../../apps/desktop/src-tauri/src/commands/server_clone.rs) — this is a mechanical port, not new logic. Full spec is Phase 5 in [`clone-auth-header-config-editing.md`](./clone-auth-header-config-editing.md); not tracked in `dev-to-main-port.md`'s Phase 6, which is unrelated desktop cloning UI.
...
- [`clone-auth-header-config-editing.md`](./clone-auth-header-config-editing.md) — clone seeding/definition-save/warning UI shipped on desktop; Phase 5 there is the active (not parked) web-admin clone parity work item 4 above tracks
