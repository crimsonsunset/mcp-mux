...
```
Enable ONLY taylorwilsdon.google-workspace-mcp-uvx (not the work clone).
...
| Other clone's tools not in results | ☑    | ☐    | Work clone inactive; search with `server_id: …-work` → `total: 0`                                    |
