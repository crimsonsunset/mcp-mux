...
**Base branch:** `dev`
**Issue:** TBD
**Depends on:** [Web Admin Mode](../guide/gateway.mdx) — transport + `command_bridge` + admin server landed on `feat/web-ui`; this doc hardens the **frontend boundary** so web/desktop regressions stop
...
- [x] Update [gateway guide](../guide/gateway.mdx) frontend section to reference facade
...
| [`docs/planning/pr-2-web-admin-code-review.md`](../../planning/pr-2-web-admin-code-review.md)                      | Post-review remediation status — what landed before facade work |
| [`docs/guide/gateway.mdx`](../guide/gateway.mdx)                                                                   | Parent feature — `command_bridge` + admin HTTP                  |
...
- [Web Admin Mode (Remote UI via HTTP)](../guide/gateway.mdx)
...
**May 26, 2026 (Phase 5):** Unified backend facade Phases 1–5 complete on `feat/web-ui`. `@/lib/backend` is the preferred import; `@/lib/api/*` shims carry deprecation comments. See [gateway guide](../guide/gateway.mdx) for the parent web-admin feature.
